...
(() => {
  ...
  // src/cdn.ts
  // Issue #137: vendored into the artifact rather than fetched from unpkg at
  // read time -- every visit was telling unpkg the visitor's IP. The SRI
  // hashes are unchanged and still verify: these are the exact same bytes
  // unpkg served for these pinned versions (compared by sha384 before
  // vendoring, not assumed), so integrity checking the vendored copy still
  // catches accidental corruption the same way it caught a substituted CDN
  // response before.
