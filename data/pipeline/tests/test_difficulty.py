"""Holds content/difficulty.json to its generator and to ADR-028's claims.
...
cannot rank. The second is that ADR-028's *measured* claims -- the ones it
...
def failures() -> list[str]:
    ...
    # 3. Reachability (ADR-028's first measured claim). Every word sits
    ...
    # 4. The ordering IS the frequency ordering (ADR-028's third measured
    ...
    # 5. A fresh reader has words to meet (ADR-028's second measured claim).
