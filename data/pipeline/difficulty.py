"""Word difficulty table — writes content/difficulty.json (ADR-028).
...
theta. ADR-028 names the falsifier and says plainly that the slope is a guess.
...
# ADR-028's one asserted constant.
LOGITS_PER_ZIPF = 1.0
...
def build() -> dict:
    ...
    if unreachable:
        raise SystemExit(
            ...
            f"LOGITS_PER_ZIPF is too steep -- ADR-028 derives its ceiling."
        )
    ...
    return {
        ...
        "note": (
            ...
            "data/pipeline/difficulty.py from wordfreq; see ADR-028. Do not "
            "hand-edit."
        ),
        "words": table,
    }
